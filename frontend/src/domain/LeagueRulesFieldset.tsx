            ...
            <div key={role}>
              <label htmlFor={`${baseId}-${role}`} className="flex min-h-6 items-center gap-1.5 text-base">
                {/* Due forme della stessa etichetta: la lettera colorata per chi guarda,
                    «Posti portieri» per chi ascolta — un solo nodo di testo, perche' lo
                    spazio fra due nodi separati si perde nel nome accessibile.
                    «Posti» e non «Slot»: l'app parla italiano. */}
                <span aria-hidden="true">Posti</span>
                <span aria-hidden="true"><RoleBadge role={role} /></span>
                <span className="sr-only">{`Posti ${name}`}</span>
              </label>
              <StepperField
                id={`${baseId}-${role}`}
                ...
                min={MIN_SLOTS}
                max={MAX_SLOTS}
                decreaseLabel={`Un posto in meno: ${name}`}
                increaseLabel={`Un posto in più: ${name}`}
                disabled={disabled}
